...
class RidgeLinearRegression:
    ...
    def _compute_grads(self, X, y):
        ...
        r = y_pred - y
        # Normalize both terms by n for stable gradient descent
        # sklearn's Ridge: ||y - Xw||² + alpha * ||w||²
        # When normalizing data loss by n, to match sklearn's alpha,
        # we need to scale lambda by n: (1/n)*||y-Xw||² + (alpha/n)*||w||²
        # So: lambda/n = alpha/n  =>  lambda = alpha
        # But since we're normalizing data loss, effective reg strength is lambda*n
        # To match sklearn's alpha=1.0, we use lambda/n in normalized version
        grad_w = (2.0 / n) * (X.T @ r) + (2.0 * self.lamb / n) * self.w
